...
export default function NewProjectPage() {
  ...
  const [mobileThumbnailFile, setMobileThumbnailFile] = useState<File | null>(
    null,
  );
  ...
  const [mobileDemoPreview, setMobileDemoPreview] = useState<string | null>(
    null,
  );
  ...
  const hasDesktop =
    selectedPlatforms.includes("Web") || selectedPlatforms.includes("Windows");
  const hasMobile =
    selectedPlatforms.includes("Android") || selectedPlatforms.includes("iOS");
    ...
    try {
      ...
      chapters.forEach((ch) =>
        ch.features.forEach((f) => {
          ...
          f.imageFiles.forEach((img) => {
            if (img.file) allFilesToUpload.push(img.file as File);
          });
        }),
      );
      ...
      const mobileThumbnailUrl = await upload(
        mobileThumbnailFile,
        "thumbnails/mobile",
      );
      ...
      for (const chapter of chapters) {
        ...
        for (const feature of chapter.features) {
          ...
          for (const img of feature.imageFiles) {
            if (img.file)
              image_urls.push(
                await upload(img.file as File, "features/images"),
              );
          }
      ...
      const techStackArray = data.tech_stack
        ? data.tech_stack.split(",").map((s) => s.trim())
        : [];
    ...
    <div className="max-w-5xl mx-auto pb-20 relative">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 space-y-6">
          <h2 className="text-xl font-bold text-white border-b border-zinc-800 pb-4 mb-6">
            المعلومات الأساسية
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                className="w-full bg-zinc-950 border border-zinc-800 text-white rounded-xl px-4 py-3 outline-none"
              />
            </div>
            <div>
              <label className="block text-zinc-400 text-sm mb-2">
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full bg-zinc-950 border border-zinc-800 text-white rounded-xl px-4 py-3 outline-none resize-y"
            />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-emerald-400 text-sm mb-3 font-medium">
                المنصات المدعومة:
              </label>
              <div className="flex flex-wrap gap-4">
                <label className="flex items-center gap-2 bg-zinc-950 border border-zinc-800 px-4 py-2 rounded-xl cursor-pointer hover:border-emerald-500 transition-colors">
                  ...
                  />{" "}
                  <Globe size={16} className="text-zinc-400" /> Web
                </label>
                <label className="flex items-center gap-2 bg-zinc-950 border border-zinc-800 px-4 py-2 rounded-xl cursor-pointer hover:border-emerald-500 transition-colors">
                  <input
                    type="checkbox"
                    ...
                    className="w-4 h-4 accent-emerald-500"
                  />{" "}
                  <Monitor size={16} className="text-zinc-400" /> Windows
                </label>
                <label className="flex items-center gap-2 bg-zinc-950 border border-zinc-800 px-4 py-2 rounded-xl cursor-pointer hover:border-emerald-500 transition-colors">
                  <input
                    type="checkbox"
                    ...
                    className="w-4 h-4 accent-emerald-500"
                  />{" "}
                  <Smartphone size={16} className="text-zinc-400" /> Android
                </label>
                <label className="flex items-center gap-2 bg-zinc-950 border border-zinc-800 px-4 py-2 rounded-xl cursor-pointer hover:border-emerald-500 transition-colors">
                  <input
                    type="checkbox"
                    ...
                    className="w-4 h-4 accent-emerald-500"
                  />{" "}
                  <Apple size={16} className="text-zinc-400" /> iOS
                </label>
              </div>
            </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8">
          <h2 className="text-xl font-bold mb-6 text-white border-b border-zinc-800 pb-4">
            الوسائط والمظهر
          </h2>
          {!hasDesktop && !hasMobile && (
            <div className="text-center py-8 text-zinc-500 border-2 border-dashed border-zinc-800 rounded-2xl bg-zinc-950">
              يرجى تحديد منصة واحدة.
            </div>
          )}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {hasDesktop && (
              <div className="space-y-4 animate-in fade-in duration-500">
                <h3 className="text-emerald-500 font-medium flex items-center gap-2">
                  <Monitor size={18} /> وسائط ال/ديسكتوب
                </h3>
                <div className="grid grid-cols-2 gap-4">
                  <div className="border-2 border-dashed border-zinc-700 rounded-2xl p-4 flex flex-col items-center justify-center text-center relative h-40 group bg-zinc-950 overflow-hidden">
                    {thumbnailPreview ? (
                      ...
                      <div className="text-zinc-500">
                        <ImageIcon size={28} className="mb-2 mx-auto" />
                    ...
                    <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4 rounded-2xl">
                      {thumbnailPreview && (
                        <button
                          ...
                          className="p-3 bg-zinc-800 hover:bg-zinc-700 text-white rounded-full transition-colors"
                        >
                          <ZoomIn size={20} />
                        </button>
                      )}
                      <label className="p-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-full transition-colors cursor-pointer">
                        <Upload size={20} />
                        <input
                          type="file"
                          accept="image/*"
                  ...
                  <div className="border-2 border-dashed border-zinc-700 rounded-2xl p-4 flex flex-col items-center justify-center text-center relative h-40 group bg-zinc-950 overflow-hidden">
                    {demoPreview ? (
                      demoPreview.match(/\.(mp4|webm)$/i) ||
                      (demoFile && demoFile.type.startsWith("video/")) ? (
                        <video
                          src={demoPreview}
                          autoPlay
                          loop
                      ...
                      <div className="text-zinc-500">
                        <Video size={28} className="mb-2 mx-auto" />
                        <span className="text-sm">فيديو العرض</span>
                      </div>
                    )}
                    <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4 rounded-2xl">
                      {demoPreview && (
                        <button
                          ...
                          onClick={() =>
                            setLightboxMedia({
                              type: demoFile?.type.startsWith("video/")
                                ? "video"
                                : "image",
                              url: demoPreview,
                            })
                      ...
                      <label className="p-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-full transition-colors cursor-pointer">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleFileChange(e, setDemoFile, setDemoPreview)
                          }
                          className="hidden"
                        />
                      </label>
            ...
            {hasMobile && (
              <div className="space-y-4 animate-in fade-in duration-500">
                <h3 className="text-blue-500 font-medium flex items-center gap-2">
                  <Smartphone size={18} /> وسائط الجوال
                </h3>
                <div className="grid grid-cols-2 gap-4">
                  <div className="border-2 border-dashed border-zinc-700 rounded-2xl p-4 flex flex-col items-center justify-center text-center relative h-40 group bg-zinc-950 overflow-hidden">
                    {mobileThumbnailPreview ? (
                      ...
                      <div className="text-zinc-500">
                        <ImageIcon size={28} className="mb-2 mx-auto" />
                        <span className="text-sm">غلاف الجوال</span>
                      </div>
                    )}
                    <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4 rounded-2xl">
                      {mobileThumbnailPreview && (
                      ...
                      <label className="p-3 bg-blue-600 hover:bg-blue-500 text-white rounded-full transition-colors cursor-pointer">
                        <Upload size={20} />
                        <input
                          type="file"
                          accept="image/*"
                          onChange={(e) =>
                            handleFileChange(
                              e,
                              setMobileThumbnailFile,
                              setMobileThumbnailPreview,
                            )
                          }
                          className="hidden"
                        />
                  ...
                  <div className="border-2 border-dashed border-zinc-700 rounded-2xl p-4 flex flex-col items-center justify-center text-center relative h-40 group bg-zinc-950 overflow-hidden">
                    {mobileDemoPreview ? (
                      mobileDemoPreview.match(/\.(mp4|webm)$/i) ||
                      (mobileDemoFile &&
                        mobileDemoFile.type.startsWith("video/")) ? (
                        <video
                          src={mobileDemoPreview}
                          autoPlay
                          loop
                          muted
                          className="w-full h-full object-cover rounded-xl"
                      ...
                      <div className="text-zinc-500">
                        <Video size={28} className="mb-2 mx-auto" />
                        <span className="text-sm">فيديو الجوال</span>
                      </div>
                    )}
                    <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4 rounded-2xl">
                      {mobileDemoPreview && (
                        <button
                          ...
                          onClick={() =>
                            setLightboxMedia({
                              type: mobileDemoFile?.type.startsWith("video/")
                                ? "video"
                                : "image",
                              url: mobileDemoPreview,
                            })
                          }
                          className="p-3 bg-zinc-800 hover:bg-zinc-700 text-white rounded-full transition-colors"
                        >
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 space-y-6">
          <h2 className="text-xl font-bold flex items-center gap-2 text-white border-b border-zinc-800 pb-4">
            <Star className="text-yellow-500" /> التميز والهوية
          </h2>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 space-y-6">
          <h2 className="text-xl font-bold flex items-center gap-2 text-white border-b border-zinc-800 pb-4">
            <LinkIcon className="text-zinc-400" /> الروابط والمتاجر
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {hasDesktop && (
              <div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 space-y-6">
          <h2 className="text-xl font-bold flex items-center gap-2 text-white border-b border-zinc-800 pb-4">
            <User className="text-zinc-400" /> العميل
          </h2>
          <div className="grid grid-cols-1 gap-6">
            ...
            <div>
              <label className="block text-zinc-400 text-sm mb-2">الرأي</label>
              <textarea
        ...
        <button
          type="submit"
          disabled={uploadState.isUploading || uploadState.isProcessingData}
          className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-2xl px-4 py-5 flex items-center justify-center gap-2 transition-colors disabled:opacity-50 text-lg shadow-[0_0_20px_rgba(16,185,129,0.3)]"
        >
          <Save
            size={24}
            className={
              uploadState.isUploading || uploadState.isProcessingData
                ? "animate-spin"
                : ""
            }
          />
